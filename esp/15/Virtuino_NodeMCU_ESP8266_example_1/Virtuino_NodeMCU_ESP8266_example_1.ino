...
void loop() {
  ...
  int v5 = virtuino.vMemoryRead(5);//гираскоп значение
  if (v5 != storedValue5) {
    Serial.println("-------Virtual pin DV5 is changed to=" + String(v5));
    mySerial.println("5:"+String(v5)+":");
    storedValue5 = v5;
  }
  int v8 = virtuino.vMemoryRead(8);//барометр значение  
  if (v8 != storedValue8) {
    Serial.println("-------Virtual pin DV8 is changed to=" + String(v8));
    mySerial.println("8:"+String(v8)+":");
    storedValue8 = v8;
  }
}
