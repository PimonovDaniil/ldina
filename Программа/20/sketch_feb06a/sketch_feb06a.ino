...
void sendData() {
  byte a1 = EEPROM.read(1);//акселерометр
  byte a2 = EEPROM.read(2);//барометр
  byte a3 = EEPROM.read(3);//зуммер
  byte a4 = EEPROM.read(4);//датчик удара
  byte a5 = EEPROM.read(5);//обрыв провода
  byte a6 = EEPROM.read(6);//гироскоп(старший байт)
  byte a7 = EEPROM.read(7);//гироскоп(младший байт)
  byte a8 = EEPROM.read(8);//барометр(старший байт)
  byte a9 = EEPROM.read(9);//барометр(младший байт)
...
void loop() {
  ...
  if((flag==false)&&(abs(oldTemper-temp)>temperZnach)){
    temperZvon==true;
  }else{
    temperZvon==false;
  }
  ...
  if (zummer == 1) {
    if ((gyroZvon) || (breakZvon) || (temperZvon)) {
      if (!zommerActive) {
        zommerActive = true;
        digitalWrite(22, LOW);
      }
